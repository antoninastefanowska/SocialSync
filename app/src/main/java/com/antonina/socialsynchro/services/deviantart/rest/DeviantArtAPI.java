...
public interface DeviantArtAPI {
    @FormUrlEncoded
    @POST("/oauth2/token")
    Call<DeviantArtAccessTokenResponse> getAccessToken(
    ...
    @FormUrlEncoded
    @POST("/oauth2/token")
    Call<DeviantArtAccessTokenResponse> refreshToken(
            @Field("client_id") String clientID,
            @Field("client_secret") String clientSecret,
            @Field("grant_type") String grantType,
    ...
    Call<DeviantArtUserResponse> whoAmI(
            @Query("expand") String expandOptions,
            @Query("access_token") String authorization);

    @GET("/api/v1/oauth2/gallery/folders")
    Call<DeviantArtGetGalleriesResponse> getGalleries(
            @Query("limit") int limit,
            @Query("offset") Integer offset,
            @Query("access_token") String authorization);

    @GET("/api/v1/oauth2/stash/publish/userdata")
    Call<DeviantArtGetUserdataResponse> getUserdata(
            @Query("access_token") String authorization);

    @GET("/api/v1/oauth2/stash/publish/categorytree")
    Call<DeviantArtGetCategoryTreeResponse> getCategoryTree(
            @Query("catpath") String catpath,
            @Query("access_token") String authorization);

    @Multipart
    @POST("/api/v1/oauth2/stash/submit")
    Call<DeviantArtStashSubmitResponse> stashSubmit(
            @Part MultipartBody.Part file,
            @Query("title") String title,
            @Query("artist_comments") String description,
            @Query("access_token") String authorization);

    @FormUrlEncoded
    @POST("/api/v1/oauth2/stash/publish")
    Call<DeviantArtStashPublishResponse> stashPublish(
            @Field("itemid") String stashID,
            @Field("is_mature") boolean isMature,
            @Field("agree_submission") boolean agreeSubmission,
            @Field("agree_tos") boolean agreeTOS,
            @Field("access_token") String authorization);

    @FormUrlEncoded
    @POST("/api/v1/oauth2/stash/delete")
    Call<DeviantArtResultResponse> stashDelete(
            @Field("itemid") String stashID,
            @Field("access_token") String authorization);

    @GET("/api/v1/oauth2/deviation/{deviation_id}")
    Call<DeviantArtDeviationResponse> getDeviation(
            @Path("deviation_id") String deviationID,
            @Query("access_token") String authorization);
}
